...
* Le nombre total d'échantillons (points qu'on a mesurés) est noté $$N$$
* Pour avoir des échantillons, il faut une fréquence d'échantillonnage $$F_e$$
* On note $$s(n)$$ la valeur du signal échantillonné pour le point $$n$$ (ici en Volt)
...
* L'amplitude fréquentielle (la valeur qui nous intéresse) est le module de la valeur complexe : $$\Vert{S(k)}\Vert$$
* On notera tout simplement $$f$$ la fréquence que l'on souhaite identifier
* La précision spectrale (fréquence entre chaque $$f_k$$) sera notée $$\delta_F$$ 

Par exemple, nous allons échantillonner un signal à $$F_e = 100kHz$$ sur 128 points ($$N = 128$$). On aura ainsi 128 mesures d'amplitudes (allant de $$S(0)$$ à $$S(127)$$) associées à 128 fréquences (allant de $$f_0$$ à $$f_{127}$$).

Maintenant que la notation est claire, nous allons pouvoir voir les formules :

1. Nous avons $$S(k) = \sum_{n=0}^N s(n)e^{(-2i{\pi}{k}\frac{n}{N})}$$ pour $$0 {\leq} k < N$$
2. Monsieur Shannon nous dit que la fenêtre d'échantillonnage doit être strictement supérieure à deux périodes du signal échantillonné. En termes mathématiques, cela signifie que : $$N{\frac{1}{F_e}}>2{\frac{1}{f}}$$
3. Il existe un lien entre le coefficient $$k$$ et sa fréquence associée $$f_k=\frac{F_e}{N}k$$
...
Prenons un exemple. Nous avons un signal à 7kHz qui arrive sur un récepteur (exemple du déclencheur d'appareil photo), et nous voulons mesurer son *amplitude* d'arrivée sur le capteur. De plus, petite contrainte supplémentaire, on va supposer que pour une future évolution du projet, nous aurons d'autres signaux qui arriveront (à 6kHz et 8kHz).
...
Le point (4) nous dit que $${\delta}_{F}=\frac{F_{e}}{N}$$. On peut donc déduire que $$\frac{F_{e}}{N}<500$$ ==> $$N>\frac{F_e}{500}=200$$. Il nous faudra donc au moins 200 points pour avoir la précision souhaitée.
...
Il ne reste plus qu'à trouver les bons coefficients ($$k$$) à calculer pour la fréquence 7kHz. En utilisant la formule du point (3), nous obtenons $$k=17,92$$. Nous ne pouvons prendre que des nombres entiers, nous allons choisir $$k=18$$, pour une fréquence de $$f_{18}=7031Hz$$.
